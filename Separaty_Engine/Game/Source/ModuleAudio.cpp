...
{
	/*DEBUG_LOG("Loading Audio Mixer");
	bool ret = true;
	SDL_Init(0);

	if(SDL_InitSubSystem(SDL_INIT_AUDIO) < 0)
	{
		DEBUG_LOG("SDL_INIT_AUDIO could not initialize! SDL_Error: %s\n", SDL_GetError());
		ret = false;
	}
	...
	if((init & flags) != flags)
	{
		DEBUG_LOG("Could not initialize Mixer lib. Mix_Init: %s", Mix_GetError());
		ret = false;
	}
...
{
	DEBUG_LOG("Freeing sound FX, closing Mixer and Audio subsystem");
...
bool ModuleAudio::PlayMusic(const char* path, float fade_time)
{
	...
	if(music == NULL)
	{
		DEBUG_LOG("Cannot load music %s. Mix_GetError(): %s\n", path, Mix_GetError());
		ret = false;
	}
	else
	{
		if(fade_time > 0.0f)
		{
			...
			{
				DEBUG_LOG("Cannot fade in music %s. Mix_GetError(): %s", path, Mix_GetError());
				ret = false;
			}
		}
		else
		{
			...
			{
				DEBUG_LOG("Cannot play in music %s. Mix_GetError(): %s", path, Mix_GetError());
				ret = false;
			}
		}
	}

	DEBUG_LOG("Successfully playing %s", path);
	return ret;*/
	return true;
}
...
unsigned int ModuleAudio::LoadFx(const char* path)
{
	...
	if(chunk == NULL)
	{
		DEBUG_LOG("Cannot load wav %s. Mix_GetError(): %s", path, Mix_GetError());
	}
	else
	{
		fx.add(chunk);
